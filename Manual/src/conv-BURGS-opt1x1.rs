        ...
        for x_block in (0..output_width as usize).step_by(block_size) {
            ...
            for y in y_block..y_max {
                for x in x_block..x_max {
                    ...
                    let mut result = 0.0;
                    general_mat_mul(1.0, &window, &kernel_array, 0.0, &mut Array2::from_shape_fn((1, 1), |_| result));
                    result = (&window * &kernel_array).sum();
                    output_array[(y, x)] = result.clamp(0.0, 255.0);
                }
            }
        }
    }
